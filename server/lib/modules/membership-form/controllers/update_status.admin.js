    ...
    return new Promise(async (resolve, reject) => {
      try {
        if (req.body.isApproved === 1) {
          ...
          <div>
              <p><b>Dear ${memberInfo.name}, </b></p>
              <p>Your membership form is rejected. Please contact at our office for further process.</p>
          </div>
          </td>
          </tr>
        `
  ...
  try {
    ...
    if (req.params.uuid) {
      ...
      if (req?.query?.formType === 'general') {
        ...
        const checkUser = await getUserInfoByUsername(req, data?.personalInformation?.email)
        ...
        const checkUser = await getUserInfoByUsername(req, data?.organizationalInformation?.email)
        if (checkUser && Object.keys(checkUser).length > 0) {
          return res.status(HTTPStatus.NOT_FOUND).json({
            status: HTTPStatus.NOT_FOUND,
        ...
        registerUersRes = await internalFun.registerUser(req, data?.organizationalInformation)
