...
module.exports = (reqBody) => {
  try {
    ...
    if (!reqBody.amount || reqBody.amount.trim().length === 0) return { isValid: false, msg: "Donation amount is required." };
    if (numberRegex.test(reqBody.amount) == false) return { isValid: false, msg: "Invalid Donation amount." };
